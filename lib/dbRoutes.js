...
export async function checkIfEmployeeIsIdentical(firstName, middleName, lastName, birthDate, position, id)
{
    try
    {
        ...
        const results =  await pool.query(query, parseInt(id));
        results.rows[0].slice(1);
        let isIdentical = true;
        if (results.rows.length > 0)
        {
            for (let i = 0; i < results.rows[0].length; i++)
            {
                if (results.rows[0][i] != values[i])
                {
                    isIdentical = false;
                }
            }
...
export async function updateEmployeeById (firstName, middleName, lastName, birthDate, position, id) 
{
    try 
    {   
        let identicalCheck = checkIfEmployeeIsIdentical(firstName, middleName, lastName, birthDate, position);
...
export async function addEmployeeToDatabase (firstName, middleName, lastName, birthDate, position) 
{
    ...
    {    
        ...
        {
            let employeeExists = checkIfEmployeeAlreadyExists(firstName, null, lastName, birthDate);
